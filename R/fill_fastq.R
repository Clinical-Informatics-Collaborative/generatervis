...
#'
#' @export
fill_fastq <- function(patient_id, sample_id, output_dir = tempdir(), n, read_length) {
  ...
  if (file.exists(file_name)) {
    ...
    create_empty_fastq(patient_id, sample_id, output_dir)
  }

  # Generate random `.fastq` reads
  fastq_reads <- rreads(patient_id, sample_id, n, read_length)
